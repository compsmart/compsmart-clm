...
class PublicToolsTest(unittest.TestCase):
    ...
    def test_client_contract(self):
        ...
        self.assertEqual(client.model()["status"], "ok")
        self.assertEqual(client.model_history()["status"], "ok")
        self.assertEqual(client.service_status()["status"], "ok")
        self.assertEqual(client.create_session()["token"], "test-session-1")
        self.assertTrue(client.chat("hello")["learned"])
        self.assertEqual(client.session_status()["status"], "ok")
        self.assertEqual(client.history()["status"], "ok")
        self.assertEqual(client.verify()["reply"], "remembered")
        self.assertEqual(client.reload()["reply"], "remembered")
